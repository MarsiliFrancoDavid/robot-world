...
RSpec.describe OrderItem do
   it "should be created if its attributes with no default value are properly provided" do
        ...
        expect(item.model_name).to eq("Ford Taunus")
    ...
    it "shouldn't be created if attributes aren't provided" do 
        ...
        expect(item.errors.full_messages).to match_array(["Order must exist", "model_name can't be blank", "Year can't be blank", "Year is not a number", "Price can't be blank", "Price is not a number", "cost_price can't be blank", "cost_price is not a number"])
    end

    it "shouldn't be created if year, price and cost_price aren't greater than 0 nor if year is older than current year" do
        order = Order.create
        item1 = OrderItem.create(model_name: "Ford Taunus",year: 0,price: 0,cost_price: -120,order_id: order.id)
        item2 = OrderItem.create(model_name: "Ford Taunus",year: Time.now.year + 1,price: 10000,cost_price: 500,order_id: order.id)
        expect(item1.errors.full_messages).to match_array(["Year must be greater than 0", "Price must be greater than 0", "cost_price must be greater than 0"])
