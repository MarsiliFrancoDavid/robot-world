...
RSpec.describe OrderItem do
   it "should be created if its attributes with no default value are properly provided" do
    ...
    it "shouldn't be created if attributes aren't provided" do 
        ...
        expect(item.errors.full_messages).to match_array(["Order must exist", "car_model_name can't be blank", "Year can't be blank", "Year is not a number", "Price can't be blank", "Price is not a number", "cost_price can't be blank", "cost_price is not a number"])
    end
